...
std::int32_t Fn1()
{
    ...
    std::int32_t*ptr =new std::int32_t{0};
    // Non-compliant - new expression
// ...
    if (errorCode != 0)
    {
        delete ptr;
...
// ...
    ...
    {
        delete ptr;
        return 0; // Memory leak could occur here
    }
// ...
...
std::int32_t Fn2()
{
    ...
    std::unique_ptr<std::int32_t> ptr2(new std::int32_t{0});
    // Non-compliant - unique_ptr provides make_unique
    // function which shall be used instead of explicit
    // new expression
    std::shared_ptr<std::int32_t> ptr3 =std::make_shared<std::int32_t>(0); // Compliant
...
{
    std::int32_t errorCode{0};
    ObjectManager<std::int32_t> manager{new std::int32_t{0}}; // Compliant by exception
    if (errorCode != 0)
    {
